...
async fn approved_context(root: &Scratch, spec: &ProcessSpec) -> ToolsContext {
...
async fn approval_invalidated_by_env_change() {
    ...
    let mut spec = shell(":");
    let context = approved_context(&root, &spec).await;
    spec.env
...
async fn approval_invalidated_when_default_workspace_changes() {
    ...
    let spec = shell(":");
    let mut context = approved_context(&root, &spec).await;
