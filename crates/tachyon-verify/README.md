...
- `run(plan, Arc<ToolsContext>, CancellationToken)` executes fresh checks through
  ...
  All cooperating workspace stages share `tachyon_tools::workspace::WorkspaceLease`.
  One canonical workspace runs one stage at a time process-wide (async
  lease held through scheduler shutdown and actual worker drain), so independent runs
  ...
  handling TERM.
- `run_with_lifetime(plan, context, cancel, Arc<dyn Send + Sync>)` additionally
  retains the caller's opaque task-owner lifetime anchor in actual workers,
  blocking snapshot scans and artifact-spool writes. It conveys no authorization.
  Aborting the outer run or its drain waiter signals cancellation but does not
  abort process cleanup; owned workers keep the lease and anchor through reap.
...
| Cancellation | Run owns its scheduler and a separate worker `JoinSet`. Dropped scheduler execution futures cancel owned workers rather than dropping their process cleanup. Explicit cancellation drains the cancellable process runner and workers before return. Dropping the whole run transfers cleanup to a drain job; actual workers retain the shared lease and optional task-owner anchor until TERM/KILL and immediate-child reap finish. Blocking scans/spool writes retain their own guard clones. Runtime/process teardown still has the upstream force-kill fallback; orderly drainage requires a live Tokio runtime. |
